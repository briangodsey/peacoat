...
## the main R script needs to run. Read the parameter
...
databasefiledir <- "../../databasefiles" # also in git repo
...
##arrayplatform <- "expressiontable"
arrayplatform <- "GEO" # specify series matrix files below
...
##                       m="GeneName")
## this has worked for Affy array data
possmir.gname <- list(mir="GeneName",
                      m="GeneName")


## ##########################
## for GEO Series Matrix files, use this
geo.files <- c(mir="../../geo-data/GSE17498-GPL8227_series_matrix.txt.gz",
               m="../../geo-data/GSE17498-GPL9021_series_matrix.txt.gz")

if( arrayplatform=="GEO" ){
  gname <- list(mir="ID",
                m="ID")
  possmir.gname <- list(mir="miRNA_ID",
                        m="ORF")
}
...
## OR: write a script to create types #####

## You can also use a script to generate this from the data, after
## xpr.orig is made.
## 
typefollows.generator <- "lionetti_typefollows_generatorscript.R"


## IF NECESSARY
## the name (and path) of an R script that adds the "type" column to
## xpr.orig[[side]]. Set to NULL if you want to ignore. For advanced
## users only.
typeadder.script <- "lionetti_type_adderscript.R"
...
### ftestthresh <- 0.05  # only if there are replicates

ftestthresh <- list()
ftestthresh$mir <- 25 #100
ftestthresh$m <- 25 #300
